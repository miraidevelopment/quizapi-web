...
app.get("/", (req, res) => {
  ...
  console.log(`[PING] Novo ping em: ${ping.getUTCHours()}:${ping.getUTCMinutes()}:${ping.getUTCSeconds()}`);
  res.sendStatus(200);
});
app.listen(process.env.PORT);
// definindo discord e node-fetch
...
app.get("/esportes/:id", async(req, res) => {
    ...
    if(sendError(id, "esportes", res)) {} else {
      ...
      res.json({
        questao: questionC1, alternativas: alternativeC1, resposta: answerC1
      });
    };
});
...
app.get("/historia/:id", async(req, res) => {
    ...
    res.json({
      questao: questionC2, alternativas: alternativeC2, resposta: answerC2
    });
   };
});
